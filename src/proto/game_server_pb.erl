...
-export([encode_game_server/1, decode_game_server/1,
	 delimited_decode_game_server/1]).
...
-record(game_server,
	{uid, name, leave, equ_name, equ_leave, tools_name,
	 tool_num}).
...
encode_game_server(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_game_server(Record)
    when is_record(Record, game_server) ->
    encode(game_server, Record).

encode(game_server, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(game_server, Record) ->
    [iolist(game_server, Record)
...
iolist(game_server, Record) ->
    [pack(1, required,
	  with_default(Record#game_server.uid, none), int32, []),
     pack(2, optional,
	  with_default(Record#game_server.name, none), string,
	  []),
     pack(3, repeated,
	  with_default(Record#game_server.leave, none), int32,
	  []),
     pack(4, repeated,
	  with_default(Record#game_server.equ_name, none), string,
	  []),
     pack(5, optional,
	  with_default(Record#game_server.equ_leave, none), int32,
	  []),
     pack(6, optional,
	  with_default(Record#game_server.tools_name, none),
	  string, []),
     pack(7, optional,
	  with_default(Record#game_server.tool_num, none), int32,
	  [])].
...
decode_game_server(Bytes) when is_binary(Bytes) ->
    decode(game_server, Bytes).

delimited_decode_game_server(Bytes) ->
    delimited_decode(game_server, Bytes).
...
decode(game_server, Bytes) when is_binary(Bytes) ->
    Types = [{7, tool_num, int32, []},
	     {6, tools_name, string, []}, {5, equ_leave, int32, []},
	     {4, equ_name, string, [repeated]},
	     {3, leave, int32, [repeated]}, {2, name, string, []},
	     {1, uid, int32, []}],
    Defaults = [{3, leave, []}, {4, equ_name, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(game_server, Decoded).
...
to_record(game_server, DecodedTuples) ->
